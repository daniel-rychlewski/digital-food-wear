        ...
        Scaffold(
            ...
            ScalingLazyColumn(
                ...
                if (isEnableCardPaymentCheckboxTicked.value) {
                    totalPriceWithTransactionFees = ceil((
                        totalPrice * (1.0 / (stripeConfig["processingFees"]!!["factor"] as Double))
                        + (stripeConfig["processingFees"]!!["addedAmount"] as Number).toDouble())
                        / 1000) * 1000 // avoid anything after comma, as not exists in IDR
        ...
        fun sendWhatsAppMessage(
            ...
            Thread {
                ...
                val jsonObject = JSONObject().apply {
                    ...
                    put("language", if (useInternationalNames) { "en" } else { Constants.WHATSAPP_TEMPLATE_LANGUAGE })
